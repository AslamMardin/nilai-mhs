  ...
  <div class="card-body py-2">
    <div class="d-flex flex-wrap align-items-center gap-3">
      ...
      <div class="d-flex gap-2 flex-wrap">
        ...
        <a href="{{ route('nilai.form-praktikum',$mataKuliah->id) }}" class="btn btn-sm btn-outline-success"><i class="bi bi-tools me-1"></i>Input Praktikum</a>
        @endif
        <a href="{{ route('laporan.rekap-mk',$mataKuliah->id) }}" class="btn btn-sm btn-outline-info"><i class="bi bi-bar-chart me-1"></i>Laporan</a>
        <a href="{{ route('nilai.pilih') }}" class="btn btn-sm btn-outline-secondary"><i class="bi bi-arrow-left me-1"></i>Kembali</a>
      </div>
    </div>
  </div>
</div>
...
@php
  ...
  $rataRata = $mahasiswaList->filter(fn($m)=>$m->nilaiAkhir->isNotEmpty())->avg(fn($m)=>$m->nilaiAkhir->first()->nilai_akhir);
@endphp
<div class="row g-3 mb-3">
  <div class="col-3"><div class="card text-center py-2"><div class="fw-700 text-primary fs-5">{{ $totalMhs }}</div><div class="text-muted small">Mahasiswa</div></div></div>
  <div class="col-3"><div class="card text-center py-2"><div class="fw-700 text-info fs-5">{{ $sudahDinilai }}</div><div class="text-muted small">Sudah Dinilai</div></div></div>
  <div class="col-3"><div class="card text-center py-2"><div class="fw-700 text-success fs-5">{{ $lulus }}</div><div class="text-muted small">Lulus</div></div></div>
  <div class="col-3"><div class="card text-center py-2"><div class="fw-700 text-warning fs-5">{{ number_format($rataRata??0,1) }}</div><div class="text-muted small">Rata-rata NA</div></div></div>
</div>
      ...
      <table class="table table-hover align-middle mb-0" style="font-size:13px">
        <thead class="table-light">
          <tr>
            ...
            @if($mataKuliah->hasTeori())
            <th colspan="5" class="text-center border-start" style="background:#f0f7ff">Nilai Teori</th>
            @endif
            @if($mataKuliah->hasPraktikum())
            <th colspan="1" class="text-center border-start" style="background:#f0fff4">Praktikum</th>
            @endif
          ...
          <tr>
            ...
            @endif
            @if($mataKuliah->hasPraktikum())
            <th class="text-center border-start" style="font-size:10px;background:#f0fff4">Nilai Prak</th>
            @endif
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              <a href="{{ route('mahasiswa.show',$mhs->id) }}" class="text-decoration-none text-dark">
            ...
            <td class="text-center">
              @if($na)
              <span class="badge {{ $na->persentase_kehadiran>=75?'bg-success':'bg-danger' }}" style="font-size:11px">{{ $na->persentase_kehadiran }}%</span>
              @else —
              @endif
            </td>
            <td class="text-center">
              @if($na)
                @if($na->persentase_kehadiran>=75)<i class="bi bi-check-circle-fill text-success"></i>
                @else<i class="bi bi-x-circle-fill text-danger" title="Kehadiran < 75%"></i>@endif
              @else —
              @endif
            </td>
            <td class="text-center border-start fw-700" style="font-size:16px">{{ $na?->nilai_akhir ?? '—' }}</td>
            <td class="text-center">
              @if($na?->huruf_mutu)
              <span class="badge badge-{{ strtolower($na->huruf_mutu) }} px-2">{{ $na->huruf_mutu }}</span>
              @else —
              @endif
            </td>
            <td class="text-center">
              @if($na)
                ...
                @elseif($na->status_kelulusan=='tidak_lulus')
                  <span class="badge bg-danger" title="{{ $na->keterangan_gagal }}">Tidak Lulus</span>
                @else<span class="badge bg-secondary">Belum</span>@endif
              @else<span class="badge bg-light text-muted border" style="font-size:11px">—</span>@endif
            </td>
          </tr>
          @empty
          <tr><td colspan="15" class="text-center text-muted py-4">Belum ada mahasiswa terdaftar.</td></tr>
          @endforelse
        </tbody>
      </table>
    </div>
